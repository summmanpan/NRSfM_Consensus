...
regu_type = {'SOFT','HARD'}; %type_regu = 1;%<--
regu_order = [1,2,4]; %odr = 3; %<--
% suman de mañana, ejecutar tal cual esto, para el hard....
type_regu = 0; 
path_save='';
% for type_r = 1:size(regu_type,2)
    type_regu = type_regu+1+1;
        ...
        for i=1:size(rot_list,2) 
            ...
            for j= 1 : size(dataname,2)
                ...
                [X_cell{i,j}, err_cell{i,j}] = get_principal_function(X,dataname{j}, ...
                                            rot_list{i},regu_order(odr),regu_type{type_regu}); 
