function X = reconstruct_h(D, rotK_th,max_ite,order_L)
...
% calculate shapes
X = BMM(X, reshape(R(3, :, :), 3, 1, []),max_ite,order_L);
...
function Y = BMM(X, R, max_ite,order_L)
% Block matrix method
%
...
L2 = zeros(3*p, size(L_t,2));% HARD CONSTRAINT
...
while cost > 1e-10 && cost2 > 1e-10 && count < max_ite 
